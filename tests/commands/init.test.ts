...
describe('init command', () => {
  ...
  test('init --default seeds content via API', async () => {
    ...
    expect(parsed.rules).toContain('boundaries')
    expect(parsed.rules).toContain('context-recovery')
    expect(parsed.rules).toContain('task-completion')
    ...
    expect(store.rules.has('boundaries')).toBe(true)
    expect(store.rules.has('context-recovery')).toBe(true)
    expect(store.rules.has('task-completion')).toBe(true)

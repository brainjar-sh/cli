...
describe('buildSeedBundle', () => {
  test('returns a valid ContentBundle with all seed content', async () => {
    ...
    expect(Object.keys(bundle.rules!)).toEqual(
      expect.arrayContaining(['boundaries', 'context-recovery', 'task-completion', 'git-discipline', 'security'])
    )

    for (const key of ['boundaries', 'context-recovery', 'task-completion', 'git-discipline', 'security']) {
      expect(bundle.rules![key].entries).toHaveLength(1)
    }
  })
